...
class GameView {
    ...
    handleSelectClue() {
        //check if other player's hand has a selected card
        const selectClue = e => {
        ...
        window.addEventListener("mousemove", selectClue)
    }
                ...
                if (card.selected) {
                    ...
                    const releaseCard = e => {
                        ...
                        if ((releasedX >= xStart && releasedX <= xEnd) && (releasedY >= yStart && releasedY <= yEnd)) {
                            ...
                            this.drawObjects(this.gameCtx)
                            this.game.handleDiscardClick(e, this.discardPositions, this.discardColors, this.gameCtx);
                        }
                        ...
                        if ((releasedX >= xStart && releasedX <= xEnd) && (releasedY >= yStart && releasedY <= yEnd)) {
                            window.removeEventListener("mousemove", dragCard)
                            this.drawObjects(this.gameCtx)
                            this.game.handlePlayClick(e, this.discardPositions, this.playPositions, this.playColors, this.discardColors, this.gameCtx);
                            this.game.addCard(this.game.currentPlayer.hand)
    ...
    drawObjects(gameCtx, won=false, over=false) {
        ...
        for (let i = 0; i < this.game.currentPlayer.hand.length; i++){
            ...
            card.secondarySelected = false;
            card.draw(this.gameCtx, card.revealedColor, card.revealedNum)
        }
        
        for (let i = 0; i < this.game.players[1].hand.length; i++) {
            let card = this.game.players[1].hand[i]
            card.pos = this.otherPlayerPositions[i]
            card.draw(this.gameCtx, true, true)
        }
